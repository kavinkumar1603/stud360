...
CREATE TYPE nptel_exam_reg_status AS ENUM ('NOT_REGISTERED', 'REGISTERED_FEE_PAID');
CREATE TYPE nptel_exam_status AS ENUM ('PENDING', 'ATTENDED', 'MISSED');
CREATE TYPE nptel_cert_type AS ENUM ('NONE', 'SUCCESSFULLY_COMPLETED', 'ELITE', 'SILVER', 'GOLD');
...
-- Create NPTEL Tracking Table
CREATE TABLE public.nptel_tracking (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  course_id uuid REFERENCES public.online_courses(id) ON DELETE CASCADE NOT NULL UNIQUE,
  exam_registration_status nptel_exam_reg_status DEFAULT 'NOT_REGISTERED',
  assignment_score numeric(5,2),
  exam_date date,
  exam_status nptel_exam_status DEFAULT 'PENDING',
  final_score numeric(5,2),
  certificate_type nptel_cert_type DEFAULT 'NONE',
...
ALTER TABLE public.nptel_tracking ENABLE ROW LEVEL SECURITY;
